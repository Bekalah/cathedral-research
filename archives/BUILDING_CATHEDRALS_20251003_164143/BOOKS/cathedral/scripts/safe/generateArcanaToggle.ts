...
type ArcanaMeta = {
  ...
  living: {
    art_realms: string[];
    synthesizer_stations?: string[];
    cathedral_zones: string[];
    temple_functions: string[];
    reiki_grid_tokens: Record<string, number>;
    atelier_exercises: string[];
  };
  provenance: {
    source_refs: string[];
  };
...
export function applyArcanaToggle(card: ArcanaMeta): Tokens {
  const styles = loadStyles();
  const synthStations = loadSynthStations();
  
  // Find matching style based on card's art realms
  const matchingStyle = styles.find((style: any) => 
    card.living.art_realms.includes(style.id)
  ) || styles.find((style: any) => 
    style.element === card.element && style.stage === card.stage
  );

  // Find synthesizer stations for this card
  const cardSynthStations = synthStations.filter((station: any) =>
    card.living.art_realms.some((realm: string) => realm === station.style_realm)
  );
  ...
  const music = {
    ...
    synthesizer_stations: cardSynthStations.map((s: any) => s.id),
  ...
  const synthesizer = {
    ...
    available_stations: cardSynthStations.map((s: any) => s.id),
    safety_level: getSafetyLevel(card),
    eeg_focus: cardSynthStations[0]?.EEG_focus || 'alpha',
    lfe_enabled: cardSynthStations.some((s: any) => s.lfe?.enable),
...
// Helper functions
function getSacredGeometry(card: ArcanaMeta) {
  const geometries = {
    'Fire': 'triangle',
    'Water': 'circle', 
    'Air': 'octagon',
  ...
  return geometries[card.element as keyof typeof geometries] || 'circle';
}

function getMusicalMode(element: string) {
  const modes = {
    'Water': 'dorian',
    'Air': 'lydian', 
    'Fire': 'mixolydian',
  ...
  return modes[element as keyof typeof modes] || 'aeolian';
}

function getStageBPM(stage: string) {
  const bpms = {
    'Nigredo': 54,
    'Albedo': 60,
    'Citrinitas': 72,
  ...
  return bpms[stage as keyof typeof bpms] || 60;
}

function getSolfeggioFreq(numerology: number) {
  const freqs = [174, 285, 396, 417, 528, 639, 741, 852, 963];
  return freqs[numerology % freqs.length];
}

function getHarmonicSeries(degree: number) {
...
function getEEGTarget(stage: string) {
  const targets = {
    'Nigredo': 'delta/theta',
    'Albedo': 'theta/alpha',
  ...
  return targets[stage as keyof typeof targets] || 'alpha';
}

function getIntensityLevel(stage: string) {
  const levels = {
    'Nigredo': 0.6,
    'Albedo': 0.8,
    'Citrinitas': 0.9,
  ...
  return levels[stage as keyof typeof levels] || 0.8;
}

function getSafetyLevel(card: ArcanaMeta) {
  if (card.stage === 'Nigredo') return 'high';
  if (card.element === 'Fire') return 'medium';
  return 'standard';
}

function getSafetyProtocols(card: ArcanaMeta) {
  return {
    contraindication_check: true,
...
function getCathedralLighting(card: ArcanaMeta) {
  return {
...
function getCathedralAmbience(card: ArcanaMeta) {
  return {
...
function filterSafeSources(sources: string[]) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log("🔮 Testing Arcana Toggle Generator");
  console.log(JSON.stringify(applyArcanaToggle(testCard), null, 2));
}